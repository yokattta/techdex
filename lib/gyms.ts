...
export const tracks: Track[] = [
  ...
  },
  {
    ...
    gap: {
      en: "The serving layer is covered now; the layer under it is not. Still missing, in rough order of how often it bites: speculative decoding, scheduling policy beyond first-come-first-served, multi-tenant isolation on shared accelerators, and the kernel-level work — fusion, attention kernels, what the compiler actually emitted — that decides whether any of the above lands.",
      zh: "服务层现在覆盖到了，它下面那一层还没有。仍然缺的，大致按「多常咬到你」排序：投机解码、先到先服务之外的调度策略、共享加速器上的多租户隔离，以及 kernel 层面的工作 —— 融合、attention kernel、编译器到底吐出了什么 —— 那些决定了上面这一切到底落不落得了地。",
    },
  },
  {
    id: "fde",
